...
- `imagegen/workflows/{1,2}/workflow.json` — two image-edit + ReActor face-swap graphs in ComfyUI **API format** (workflow 1 = Flux 2, workflow 2 = Qwen-Image-Edit-2511); each `config.json` lists, positionally, the nodes a template may customize.
- `imagegen/templates/{1,2}/config.json` — two parallel render templates. The worker renders **every** story through the live one, `templates/2` (`_RENDER_TEMPLATE_ID = "2"` in `model.py`); `templates/1` ships alongside as the legacy/alternate, so re-pointing the worker is a one-line change. Each **panel** is a list of `{field: value}` presets parallel to its workflow's node list (input image, face source, prompt, per-panel seed, per-panel `P<n>_V1`/`P<n>_V2` filename prefixes). Each ships **six** panels (one ComfyUI run each); the panels' prompt fields are filled at prepare time from the job's prompt set. Image filenames carry `USER_ID` / `STORY_ID` placeholders (e.g. `USER_ID_STORY_ID_INPUT_1.png`) that resolve to the per-story name the worker uploads the photo under — so the rendered `LoadImage` node references exactly what ComfyUI stored.
...
2. Load + validate the live render template (`templates/2`) + its workflow and apply `prompts/<type>_<id>.json` to the panels (missing or malformed asset, or a prompt/panel count mismatch → `UnsupportedTemplateError`); every panel must line up positionally with the workflow config's node list. *(eager)*
3. Upload each distinct input slot once, **under its substituted per-story filename**. The workflow points both LoadImage nodes at the same input slot, so a single photo fills the base image *and* the face-swap source. *(eager)*
